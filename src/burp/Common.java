...
public class Common {
    ...
    public static String getRespBody(IHttpRequestResponse reqres) {
        byte[] respBytes = reqres.getResponse();
        if (respBytes == null) return "";

        // 查找 header-body 分隔符（\r\n\r\n）
        for (int i = 0; i < respBytes.length - 3; i++) {
            if (respBytes[i] == '\r' && respBytes[i + 1] == '\n' &&
                    respBytes[i + 2] == '\r' && respBytes[i + 3] == '\n') {
                // body starts at i + 4
                byte[] bodyBytes = Arrays.copyOfRange(respBytes, i + 4, respBytes.length);
                return new String(bodyBytes); // 可加 charset 解码，如 UTF-8
            }
        }
        return "";
    }
